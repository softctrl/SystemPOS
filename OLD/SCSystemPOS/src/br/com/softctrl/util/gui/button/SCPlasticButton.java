...
public class SCPlasticButton extends PlasticButtonUI {

	public SCPlasticButton() {
		super();
	}

	static SCPlasticButton ui = new SCPlasticButton();
	...
	public void update(Graphics g, JComponent c) {
		if (c.isOpaque()) {
			...
			if (isToolBarButton(b)) {
				...
				if (is3D(b)) {
					Rectangle r = new Rectangle(1, 1, c.getWidth() - 2,
							c.getHeight() - 1);
					add3DEffekt(g, r);
				}
			}
	...
	private static void add3DEffekt(Graphics g, Rectangle r,
			boolean isHorizontal, Color startC0, Color stopC0, Color startC1,
			Color stopC1) {
	...
	static void add3DEffekt(Graphics g, Rectangle r) {
		...
		Graphics2D g2 = (Graphics2D) g;
		int border = 10;
		g2.setPaint(new GradientPaint(r.x, r.y, brightenStop, r.x + border,
				r.y, PlasticTheme.BRIGHTEN_START));
		g2.fillRect(r.x, r.y, border, r.height);
		int x = r.x + r.width - border;
		int y = r.y;
		g2.setPaint(new GradientPaint(x, y, PlasticTheme.DARKEN_START, x
				+ border, y, PlasticTheme.LT_DARKEN_STOP));
		g2.fillRect(x, y, border, r.height);

		add3DEffekt(g, r, true, PlasticTheme.BRIGHTEN_START, brightenStop,
	...
	static void addLight3DEffekt(Graphics g, Rectangle r, boolean isHorizontal) {
		...
		add3DEffekt(g, r, isHorizontal, PlasticTheme.BRIGHTEN_START,
				ltBrightenStop, PlasticTheme.DARKEN_START,
	...
	public static void addLight3DEffekt(Graphics g, Rectangle r) {
		...
		add3DEffekt(g, r, true, PlasticTheme.DARKEN_START,
				PlasticTheme.LT_DARKEN_STOP, PlasticTheme.BRIGHTEN_START,
				ltBrightenStop);
	}
